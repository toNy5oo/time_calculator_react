...
import { Avatar, Button, Drawer, InputNumber, List, Popconfirm, Tag } from 'antd'
...
const DrawerTablesOnHold = ({
    onClose, open, holdTables, showSharedBill, setHoldTableClicked,
    holdTableClicked,  handleChangeInput, sharedBill, setShowSharedBill,
    closeHoldTable
}) => {
  return (
        <Drawer
				...
				<List
					...
					renderItem={(item) => (
						<List.Item
							actions={[
								...
								<Popconfirm
									...
									placement={"left"}
									onConfirm={() => closeHoldTable(item.tableNumber)}
									okText="Yes"
									cancelText="No"
								>
						...
						>
							<List.Item.Meta
								avatar={
									<Avatar src={require(`../assets/img/${item.tableNumber}ball.png`)}/>
								}
